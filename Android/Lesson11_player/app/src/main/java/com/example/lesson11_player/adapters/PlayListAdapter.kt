...
class PlayListAdapter(private val songs: List<Song>,
    ...
    class PlayListViewHolder(private val view: View, private val songs: List<Song>): RecyclerView.ViewHolder(view) {
        public fun bind(song: Song, artistClickHandler: (Song) -> Unit) {
            ...
            when(song.state) {
                SongState.PLAY -> playBtn.setImageDrawable(view.resources.getDrawable(R.drawable.pause, null))
                SongState.PAUSE -> playBtn.setImageDrawable(view.resources.getDrawable(R.drawable.play_circle, null))
            }
            playBtn.setOnClickListener{_ ->
                songs.forEach{s ->
    ...
    override fun onBindViewHolder(holder: PlayListAdapter.PlayListViewHolder, position: Int) {
        holder.bind(songs.get(position), artistClickHandler)
        return
    }
